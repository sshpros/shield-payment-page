...
const response = await fetch(
  `${supabaseUrl}/rest/v1/invoices?id=eq.${invoice_id}&select=*`,
  {
    headers: {
      apikey: supabaseKey,
...
.logo h1 { font-size: 22px; font-weight: 700; color: #4A90D9; }
...
.invoice-info .row { display: flex; justify-content: space-between; margin-bottom: 8px; }
...
.invoice-info .total .value { color: #4A90D9; font-size: 20px; font-weight: 700; }
.field-label { font-size: 13px; color: #8B949E; margin-bottom: 6px; display: block; }
.collect-field {
  background: #0D1117; border: 1px solid #30363D; border-radius: 10px;
  padding: 14px; margin-bottom: 16px; min-height: 48px;
}
#pay-btn {
  width: 100%; padding: 16px; background: #4A90D9; color: #FFFFFF;
...
<div class="invoice-info">
  <div class="row">
    ...
    <span class="value">${invoice.invoice_number || invoice_id.slice(0, 8)}</span>
  </div>
  <div class="row">
...
<button id="pay-btn" onclick="submitPayment()">Pay $${Number(invoice.balance_due).toFixed(2)}</button>
<div id="status"></div>
</div>
